...
from pythologist.image_utilities import watershed_image, map_image_ids, generate_new_region_image
import sys, os, io
import imageio
    ...
    def get_segmentation_map_images(self,type='edge',subset_logic=None,color=None,watershed_steps=0,blank=(0,0,0,255)):
        ...
        for i,r in self.iterrows():
            ...
            if color is not None:
                ...
                #corods=np.array(map_image_ids(img)[['y','x']].apply(lambda x: tuple(x),1).tolist())
                fresh[coords.T[:,0],coords.T[:,1]] = color
...
def phenotypes_to_regions(cdf,region_group_name,path=None,
    ...
    for _f in cdf.db.frame_iter():
        break
    _temp_name = _f.get_data('region_groups').iloc[0]['region_group']
    _temp = output.cdf(region_group=_temp_name)
...
def fetch_single_segmentation_image_bytes(self,schema,background=(0,0,0,255),tempdir=None):
    ...
    sio = self.segmentation_images().build_segmentation_image(schema,background=background)
